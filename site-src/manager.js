...
try {
  ...
  console.warn('marked.use failed (Safari?):', e);
}
...
export async function putFile(token, path, content, sha, message, { isAsset = false } = {}) {
  const pathCheck = isAsset ? isAssetPathSafe(path) : isPathSafe(path);
  if (!pathCheck) throw new Error("Unsafe vault path rejected.");
...
async function handleDelete(path, shaHint) {
  ...
  try {
    ...
    const sha = shaHint || (foundNote ? foundNote.sha : null) || await getFileSha(token, path);
